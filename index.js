...
async function run() {
  try {
    ...
    const coffeeCollection = client.db('coffeeDBLenovo').collection('coffee')
    ...
    //user
    const userCollection = client.db('coffeeDBLenovo').collection('user')
...
app.get('/',(req,res)=>{
    res.send('Coffe master coming')
})

app.listen(port,()=>{
    console.log(`Coffee master is running on ${port}`)
})